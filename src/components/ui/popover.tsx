...
>(({ className, align = "center", sideOffset = 4, showArrow = false, isPortaled = true, ...props }, ref) => {
  const content = (
    ...
    >
      {props.children}
      {showArrow && <PopoverPrimitive.Arrow className="fill-popover drop-shadow-md" width={20} height={10} />}
    </PopoverPrimitive.Content>
  );

  return isPortaled ? <PopoverPrimitive.Portal>{content}</PopoverPrimitive.Portal> : content;
});
